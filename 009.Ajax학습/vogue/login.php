...
<body>
    ...
    <!-- 
        ...
        -action속성: 서버전송 페이지 주소
        -method속성: 전송방식 선택(get/post)
        ...
        -type속성 : 여러가지 입력양식을 선택함
            1) text -글자입력
            2)password -비밀번호 입력(입력시 글자숨김)
            ->비밀번호는 영역복사도 안됨!
            3)radio -라디오 버튼(보통 단일선택시 사용)
            ...
            [ input의 공통속성 ]
            1.maxlength: 최대입력 글자수 지정
            2.placeholder :입력창의 아내문구를 출력(입력순간 글자는 사라짐)
            3. value: 미리 입력하고자 하는 텍스트를 넣음
                        (버튼일 경우 버튼 글자로 출력됨!)
            4.disabled: 값없이 바로 속성명만 사용해서 입력금지상태로 만든다
            ...
            ->action에 지정된 서버전송 페이지에서 입력값을 읽을때
             ...
             [ label 요소 ]
             -input요소의 설명을 표시함
             -라벨요소는 웹접근성의 필수 요쇼임(반드시 표시할것)
             -for속성 : input요소의 id명과 같아서 라벨클릭시 입력요소로 포커스가 자동으로 들어간다
